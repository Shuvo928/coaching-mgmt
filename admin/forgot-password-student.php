...
if(isset($_POST['send_otp'])) {
    ...
    if(empty($email)) {
        $error = 'Please enter your registered student email.';
    } else {
        // Check if student exists with this email
        $query = "SELECT s.id, s.email, s.phone, s.first_name, s.last_name, u.id as user_id
                  FROM students s
                  LEFT JOIN users u ON s.user_id = u.id
                  WHERE s.email = '$email' AND s.status = 1 AND u.status = 1";
        ...
        if(mysqli_num_rows($result) === 1) {
            ...
            $subject = 'CoachingPro Student Password Reset OTP';
            ...
            if (!empty($student['email'])) {
                $sendStatus = smtpSendMail($student['email'], $subject, $message);
                if ($sendStatus === true) {
                    $emailSent = true;
                }
            }
            ...
            if (!empty($student['phone'])) {
                ...
                $smsSent = sendSMS($student['phone'], $smsMessage);
            }

            if ($emailSent || $smsSent) {
                $_SESSION['forgot_student_id'] = $student['user_id'];
                $_SESSION['forgot_student_email'] = $student['email'];
                $_SESSION['forgot_student_phone'] = $student['phone'];
            ...
            $error = 'No active student account was found for that email.';
        }
    }
} elseif(isset($_POST['verify_otp'])) {
    $enteredOtp = trim($_POST['otp'] ?? '');
    $email = $_SESSION['forgot_student_email'] ?? '';
    ...
    if(empty($enteredOtp)) {
        ...
        unset($_SESSION['forgot_student_id'], $_SESSION['forgot_student_email'], $_SESSION['forgot_student_phone'], $_SESSION['forgot_otp'], $_SESSION['forgot_otp_expires'], $_SESSION['forgot_verified']);
        $step = 'email';
    ...
    if(!isset($_SESSION['forgot_student_id'], $_SESSION['forgot_student_email'])) {
        ...
        // Send email if available
        if (!empty($_SESSION['forgot_student_email'])) {
            $sendStatus = smtpSendMail($_SESSION['forgot_student_email'], $subject, $message);
            if ($sendStatus === true) {
                $emailSent = true;
            }
        }
        ...
        // Send SMS if phone available
        if (!empty($_SESSION['forgot_student_phone'])) {
            $smsMessage = "CoachingPro: Your password reset OTP is $otp. Valid for 5 minutes.";
            $smsSent = sendSMS($_SESSION['forgot_student_phone'], $smsMessage);
        }
    ...
    if(empty($password) || empty($confirmPassword)) {
        ...
        $studentId = $_SESSION['forgot_student_id'] ?? 0;
        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
        $update = "UPDATE users SET password = '$hashedPassword' WHERE id = $studentId AND role = 'student'";
...
<head>
    ...
    <title>Forgot Password - Student - CoachingPro</title>
...
<body>
    <div class="reset-container">
        <div class="reset-header">
            ...
            <p>Use your registered student email to reset your password.</p>
        </div>
            ...
            <?php if($step === 'email' || $step === 'completed'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        <label for="email" class="form-label">Registered Student Email</label>
                        <input type="email" id="email" name="email" class="form-control" placeholder="student@example.com" value="<?php echo sanitize($email); ?>" required>
                    </div>
            ...
            <?php elseif($step === 'otp'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" value="<?php echo sanitize($_SESSION['forgot_student_email'] ?? $email); ?>" readonly>
                    </div>
                    <div class="form-group">
