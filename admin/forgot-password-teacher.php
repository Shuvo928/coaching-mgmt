...
if(isset($_POST['send_otp'])) {
    ...
    if(empty($email)) {
        $error = 'Please enter your registered teacher email.';
    } else {
        // Check if teacher exists with this email
        $query = "SELECT t.id, t.email, t.phone, t.first_name, t.last_name, u.id as user_id
                  FROM teachers t
                  LEFT JOIN users u ON t.user_id = u.id
                  WHERE t.email = '$email' AND t.status = 1 AND u.status = 1";
        ...
        if(mysqli_num_rows($result) === 1) {
            ...
            $subject = 'CoachingPro Teacher Password Reset OTP';
            ...
            if (!empty($teacher['email'])) {
                $sendStatus = smtpSendMail($teacher['email'], $subject, $message);
                if ($sendStatus === true) {
                    $emailSent = true;
                }
            }
            ...
            if (!empty($teacher['phone'])) {
                ...
                $smsSent = sendSMS($teacher['phone'], $smsMessage);
            }

            if ($emailSent || $smsSent) {
                $_SESSION['forgot_teacher_id'] = $teacher['user_id'];
                $_SESSION['forgot_teacher_email'] = $teacher['email'];
                $_SESSION['forgot_teacher_phone'] = $teacher['phone'];
            ...
            $error = 'No active teacher account was found for that email.';
        }
    }
} elseif(isset($_POST['verify_otp'])) {
    $enteredOtp = trim($_POST['otp'] ?? '');
    $email = $_SESSION['forgot_teacher_email'] ?? '';
    ...
    if(empty($enteredOtp)) {
        ...
        unset($_SESSION['forgot_teacher_id'], $_SESSION['forgot_teacher_email'], $_SESSION['forgot_teacher_phone'], $_SESSION['forgot_otp'], $_SESSION['forgot_otp_expires'], $_SESSION['forgot_verified']);
        $step = 'email';
    ...
    if(!isset($_SESSION['forgot_teacher_id'], $_SESSION['forgot_teacher_email'])) {
        ...
        // Send email if available
        if (!empty($_SESSION['forgot_teacher_email'])) {
            $sendStatus = smtpSendMail($_SESSION['forgot_teacher_email'], $subject, $message);
            if ($sendStatus === true) {
                $emailSent = true;
            }
        }
        ...
        // Send SMS if phone available
        if (!empty($_SESSION['forgot_teacher_phone'])) {
            $smsMessage = "CoachingPro: Your password reset OTP is $otp. Valid for 5 minutes.";
            $smsSent = sendSMS($_SESSION['forgot_teacher_phone'], $smsMessage);
        }
    ...
    if(empty($password) || empty($confirmPassword)) {
        ...
        $teacherId = $_SESSION['forgot_teacher_id'] ?? 0;
        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
        $update = "UPDATE users SET password = '$hashedPassword' WHERE id = $teacherId AND role = 'teacher'";
...
<head>
    ...
    <title>Forgot Password - Teacher - CoachingPro</title>
...
<body>
    <div class="reset-container">
        <div class="reset-header">
            ...
            <p>Use your registered teacher email to reset your password.</p>
        </div>
            ...
            <?php if($step === 'email' || $step === 'completed'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        <label for="email" class="form-label">Registered Teacher Email</label>
                        <input type="email" id="email" name="email" class="form-control" placeholder="teacher@example.com" value="<?php echo sanitize($email); ?>" required>
                    </div>
            ...
            <?php elseif($step === 'otp'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" value="<?php echo sanitize($_SESSION['forgot_teacher_email'] ?? $email); ?>" readonly>
                    </div>
                    <div class="form-group">
