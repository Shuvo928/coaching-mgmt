...
if(isset($_POST['send_otp'])) {
    ...
    if(empty($email)) {
        $error = 'Please enter your registered parent email.';
    } else {
        // Check if parent exists with this email
        $query = "SELECT id, parent_name, parent_email, parent_phone, username
                  FROM parents
                  WHERE parent_email = '$email' AND status = 'Active'";
        ...
        if(mysqli_num_rows($result) === 1) {
            ...
            $subject = 'CoachingPro Parent Password Reset OTP';
            ...
            if (!empty($parent['parent_email'])) {
                $sendStatus = smtpSendMail($parent['parent_email'], $subject, $message);
                if ($sendStatus === true) {
                    $emailSent = true;
                }
            }
            ...
            if (!empty($parent['parent_phone'])) {
                $smsMessage = "CoachingPro: Your password reset OTP is $otp. Valid for 5 minutes.";
                $smsSent = sendSMS($parent['parent_phone'], $smsMessage);
            }

            if ($emailSent || $smsSent) {
                $_SESSION['forgot_parent_id'] = $parent['id'];
                $_SESSION['forgot_parent_email'] = $parent['parent_email'];
                $_SESSION['forgot_parent_phone'] = $parent['parent_phone'];
                ...
                $step = 'otp';
                $success = 'OTP sent to your registered email and/or phone. Please enter it below to reset your password.';
            } else {
                $error = 'Unable to send OTP to your email or phone. Please contact admin.';
            }
        } else {
            $error = 'No active parent account was found for that email.';
        }
    }
} elseif(isset($_POST['verify_otp'])) {
    $enteredOtp = trim($_POST['otp'] ?? '');
    $email = $_SESSION['forgot_parent_email'] ?? '';

    if(empty($enteredOtp)) {
        $error = 'Please enter the OTP sent to your email/phone.';
        $step = 'otp';
    } elseif(!isset($_SESSION['forgot_otp'], $_SESSION['forgot_otp_expires']) || time() > $_SESSION['forgot_otp_expires']) {
        $error = 'OTP has expired. Please request a new OTP.';
        unset($_SESSION['forgot_parent_id'], $_SESSION['forgot_parent_email'], $_SESSION['forgot_parent_phone'], $_SESSION['forgot_otp'], $_SESSION['forgot_otp_expires'], $_SESSION['forgot_verified']);
        $step = 'email';
    ...
    if(!isset($_SESSION['forgot_parent_id'], $_SESSION['forgot_parent_email'])) {
        ...
        // Send email if available
        if (!empty($_SESSION['forgot_parent_email'])) {
            $sendStatus = smtpSendMail($_SESSION['forgot_parent_email'], $subject, $message);
            if ($sendStatus === true) {
                $emailSent = true;
            }
        }
        ...
        // Send SMS if phone available
        if (!empty($_SESSION['forgot_parent_phone'])) {
            $smsMessage = "CoachingPro: Your password reset OTP is $otp. Valid for 5 minutes.";
            $smsSent = sendSMS($_SESSION['forgot_parent_phone'], $smsMessage);
        }
        ...
        if ($emailSent || $smsSent) {
            ...
            $_SESSION['forgot_otp_last_send'] = time();
            $_SESSION['forgot_verified'] = false;
            $success = 'OTP sent to your registered email and/or phone. Please enter it below.';
            $step = 'otp';
        } else {
            $error = 'Unable to resend OTP. Please contact admin.';
            $step = 'otp';
        }
    }
    ...
    if(empty($password) || empty($confirmPassword)) {
        ...
        $parentId = $_SESSION['forgot_parent_id'] ?? 0;
        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
        $update = "UPDATE parents SET password_hash = '$hashedPassword' WHERE id = $parentId";
...
<head>
    ...
    <title>Forgot Password - Parent - CoachingPro</title>
...
<body>
    <div class="reset-container">
        <div class="reset-header">
            ...
            <p>Use your registered parent email to reset your password.</p>
        </div>
        <div class="reset-body">
            ...
            <?php if($step === 'email' || $step === 'completed'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        <label for="email" class="form-label">Registered Parent Email</label>
                        <input type="email" id="email" name="email" class="form-control" placeholder="parent@example.com" value="<?php echo sanitize($email); ?>" required>
                    </div>
                ...
                <?php if($step === 'completed'): ?>
                    <div class="back-link">
                        <a href="parent-login.php"><i class="fas fa-sign-in-alt me-1"></i>Return to login</a>
                    </div>
                <?php endif; ?>
            <?php elseif($step === 'otp'): ?>
                <form method="POST" action="" novalidate>
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" value="<?php echo sanitize($_SESSION['forgot_parent_email'] ?? $email); ?>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="otp" class="form-label">Enter OTP</label>
                        <input type="text" id="otp" name="otp" class="form-control" maxlength="6" placeholder="6-digit code" required>
                        <div class="help-text">OTP is valid for 5 minutes. Check your email and/or SMS.</div>
                    </div>
            ...
            <div class="back-link">
                <a href="parent-login.php"><i class="fas fa-arrow-left me-1"></i>Back to Login</a>
            </div>
        </div>
    </div>
